<?php
$user=$_GET['user'];
$conn=mysqli_connect("localhost","id18320683_root","LPL.7e-ffPf%PY8","id18320683_foodexpress");;
if(!$conn){
    die("Couldn't connect to the database due to ".mysqli_connect_error());
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Profile</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Birthstone+Bounce:wght@500&family=Pacifico&display=swap" rel="stylesheet">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Oswald:wght@300&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.15.4/css/all.css" integrity="sha384-DyZ88mC6Up2uqS4h/KRgHuoeGwBcD4Ng9SiP4dIRy0EXTlnuz47vAwmeGwVChigm" crossorigin="anonymous">
    <style>
        *{
            box-sizing: border-box;
            margin: 0;
            padding: 0;
        }
    
        .logo{
            height: max-content;
            text-align: center;
            background-color: darkcyan;
            color:white;
            font-family: 'Birthstone Bounce', cursive;
            word-spacing: 10px;
            font-size: 70px;
            padding:18px ;
        }
        .nav .rand{
            text-align: center;
        }
        .nav .rand ul{
            list-style: none;
            padding: 10px 0px;
            
        }
        .nav .rand ul li{
            display: inline;
            text-align: center;
            margin: 15px;
        }
        .nav .rand ul li a{
            text-decoration:none;
            text-align: center;
            color:darkcyan;
            font-family:'Oswald';
            font-size: large;
            font-weight: bolder;
            
            
        }
        hr{
            border: 2px solid darkcyan;
        }
        /* footer .social a img{
            width: 160px;
            height: 100px;
        } */
        footer .social a i{
            color:white;
            font-size:xx-large;
            margin-right:8px;
        }
        footer{
            margin-top: 40px;
            bottom:0;
            position:relative;
            width:100%;
            background-color: darkcyan;
        }
        footer h3{
            padding-top: 10px;
            color: white;
            font-family:'Segoe UI';
        }
        footer .social,footer h3{
            text-align: center;
        }
        /* #insta{
            width: 100px;
        } */
        .copyright{
            margin-top: 8px;
        }
        .copyright p{
            padding: 5px;
            color:white;
            text-align: center;
            font-weight: bold;
            font-size: medium;
            font-family:'Segoe UI';
        }
        .profile{
            display:flex;
            flex-direction: row;
            width: 100%;
            background-color: lightgrey;
            height:max-content;
        }
        .icon{
            padding:10px;
            width:30%;
        }
        .icon img{
            width: 50%;
            float:right;
            padding-right:10px;
            display:block;
        }
        .details{
            display:block;
            margin:auto;
            float:left;
            width:33.33%;
        }
        .details p,span{
            color:darkcyan;
            font-weight:bold;
            font-family:'Segoe UI';
            font-size:larger;
        }
        .orders{
            text-align:center;
            min-height:60vh;
            max-height:max-content;
        }
        table{
            width:max-content;
            margin:auto;
            display:block;
            font-family:'Segoe UI';
            border:1px solid darkcyan;
        }
        h2{
            color:darkcyan;
            font-family:'Segoe UI';
            padding-bottom:15px;
            margin-top:10px;
        }
        th,td{
            color:darkcyan;
            font-size:medium;
            padding:6px;
        }
        th{
            
            border-bottom:1px solid darkcyan;
        }
        .btn{
            width:100%;
            text-align:right;
            display:block;
            margin:auto;
        }
        .btn button{
            border: 2px solid darkcyan;
            color: darkcyan;
            background-color: white;
            padding:7px;
            font-size:medium;
            margin-right:7px;
            border-radius:4px;
            font-weight:bold;
            text-align:center;
        }
        .btn button:hover{
            cursor:pointer;
            background-color:darkcyan;
            color:white;
        }
    </style>
</head>
<body>
    <div class="maindiv">
        <div class="banner">
            <p class="logo">Food Express</p>
        </div>
        <div class="nav">
            <div class="rand">
                <ul>
                    <?php
                    echo"<li><a href='home.php?user=$user'>Home</a></li>
                    <li><a href='aboutus.php?user=$user'>About Us</a></li>
                    <li><a href='#footer'>Contact Us</a></li>
                    <li><a href='profile.php?user=$user'>Profile</a></li>
                    ";
                    ?>
                </ul>
            </div>
            <hr>
        </div>
    </div>
    <?php 
        $sql="SELECT * from customer where User_Name='$user'";
        $result = $conn->query($sql);
        while($row = $result->fetch_assoc()){
            $name=$row['Name'];
            $email=$row['Email'];
            $phone=$row['Ph_No'];
            $id=$row['Id'];
        }
                
     ?>
    <div class="profile">
            <div class="icon">
                <img src="../images/user.png" alt="User Icon">
            </div>
            <div class="details">
                <p><?php echo$name; ?></p>
                <span><?php echo$phone; ?>   </span>
                <span><?php echo$email; ?>   </span>
            </div>
            <div class="btn">
                <form action="editProfile.php?user=<?php echo$user;?>" method="POST">
                    <?php
                        echo"<input type='hidden' name='id' value='$id'>";
                    ?>
                    <button type="submit">Edit <i class="fas fa-edit"></i></button>
                </form>
                
            </div>
    </div>
    <div class="orders">
        <h2>Your Orders</h2>
        <table>
            <tbody>
                <tr>
                    <th>Item Name</th>
                    <th>Quantity</th>
                    <th>Price</th>
                    <th>Restaurant Name</th>
                    <th>Order Date</th>
                    <th>Order Status</th>
                </tr>
                <?php
                $sql1="SELECT * from order_details where Customer_Id='$id'";
                $result=$conn->query($sql1);
                while($row=$result->fetch_assoc()){
                    $sql2="SELECT * from menu where Id={$row['Menu_Id']}";
                    $result1=$conn->query($sql2);
                    $sql3="SELECT * from site_info where site_info.Id={$row['Processed_By']}";
                    $result2=$conn->query($sql3);
                    while($row1=$result1->fetch_assoc()){
                        while($row2=$result2->fetch_assoc()){
                        echo"
                        <tr>
                        <td>{$row1['Name']}</td>
                        <td>{$row['Qty']}</td>
                        <td>{$row['Price']}</td>
                        <td>{$row2['Name']}</td>
                        <td>{$row['Order_Date']}</td>
                        <td>{$row['Order_Status']}</td>
                        </tr>
                        ";
                    }
                }
                }
                ?>
            </tbody>
        </table>
    </div>
    <footer id="footer">
        <h3>Connect With Us</h3>
        <br>
        <div class="social">
            <a href="https://www.instagram.com/foodexpressjiit/" target="_blank"><i class="fab fa-instagram"></i></a>
            <a href="https://www.facebook.com/Food-Express-103696265440982" target="_blank"><i class="fab fa-facebook"></i></a>
            <a href="mailto: foodexpressjiit@gmail.com" target="_blank"><i class="fas fa-envelope"></i></a>
        </div>
        <div class="copyright">
            <p>
                &copy;2021, Food Express
            </p>
        </div>
    </footer>
</body>
</html>